...
class GPTSovits:
    ...
    def __init__(self, config):
        ...
        self.t2s_model, self.max_sec = self.change_gpt_weights(resources.GPT_LIST[0])
        self.cache = {}
    ...
    def generate_audio(self, request: CliRequest):
        ...
        audio_opt = []
        if not request.ref_free:
            phones1, bert1, norm_text1 = self.get_phones_and_bert(request.ref_text, request.ref_language, version)
        ...
        for i_text, text in enumerate(texts):
            ...
            phones2, bert2, norm_text2 = self.get_phones_and_bert(text, request.text_language, version)
    ...
    def get_phones_and_bert(self, text, language, version, device, is_half, final=False):
        if language in {"en", "all_zh", "all_ja", "all_ko", "all_yue"}:
            ...
            for i in range(len(textlist)):
                ...
                bert = cli_util.get_bert_inf(phones, word2ph, norm_text, lang, self.is_half, self.device)
